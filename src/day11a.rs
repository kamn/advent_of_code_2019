...
fn  run_computer<F>(mem : &mut HashMap<i64, i64>, mut func: F) -> Vec<i64>  where F: FnMut(i32) -> i32 {
...
pub fn calc() -> i64 {
    ...
    //let output_mem = run_computer(mem, testf);
...
fn test_callback<F>(mut func: F) -> i32 
    where F: FnMut() -> i32{ 
    let mut cont = true;
    while cont {
        let result = func();
...
#[test]
fn test_day11a_example2() {
    let mut test = 0;
    let mut testf = || {
        test = test + 1;
        test
    };
    let result = test_callback(testf);
